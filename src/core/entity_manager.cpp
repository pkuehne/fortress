...
EntityId EntityManager::createWallPrefab (Location& location)
{
    ...
    SpriteComponent* l_sprite = 
        m_engine->state()->components()->make<SpriteComponent>(l_entity);
    l_sprite->fgColor    = Color (GREY);
    l_sprite->bgColor    = Color (BLACK);
    l_sprite->sprite     = 'W';

    // Collider Component
    m_engine->state()->components()->make<ColliderComponent>(l_entity);

    DescriptionComponent* l_description = 
        m_engine->state()->components()->make<DescriptionComponent>(l_entity);
...
EntityId EntityManager::createPlayerPrefab (Location& location)
{
    ...
    HealthComponent* l_health = 
        m_engine->state()->components()->make<HealthComponent>(l_entity);
    l_health->health = 10;

    // Player Component
    m_engine->state()->components()->make<PlayerComponent>(l_entity);

    // Euipment Component
    EquipmentComponent* l_equipment = 
        m_engine->state()->components()->make<EquipmentComponent>(l_entity);
    Location nowhere;
...
EntityId EntityManager::createEnemyPrefab (Location& location)
{
    ...
    // NPC Component
    NpcComponent* l_npc = 
        m_engine->state()->components()->make<NpcComponent>(l_entity);
...
EntityId EntityManager::createStairPrefab (STAIR dir, Location& location)
{
    ...
    StairComponent* l_stair = 
        m_engine->state()->components()->make<StairComponent>(l_entity);
...
EntityId EntityManager::createWeaponPrefab (Location& location)
{
    ...
    WieldableComponent* l_wieldable = 
        m_engine->state()->components()->make<WieldableComponent>(l_entity);
    ...
    m_engine->state()->components()->make<DroppableComponent>(l_entity);
...
EntityId EntityManager::createHelmetPrefab (Location& location)
{
    ...
    WearableComponent* l_wearable = 
        m_engine->state()->components()->make<WearableComponent>(l_entity);
...
EntityId EntityManager::createPotionPrefab (Location& location)
{
    ...
    ConsumableComponent* l_consumable = 
        m_engine->state()->components()->make<ConsumableComponent>(l_entity);
