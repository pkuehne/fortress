...
Location FileLoader::loadLocation(unsigned int& pos) {
    ...
    for (unsigned int ii = 0; ii < 4; ii++) {
        if (m_tags[pos].getName() == "LOCATION_X")
            location.x = m_tags[pos].getNum();
        if (m_tags[pos].getName() == "LOCATION_Y")
            location.y = m_tags[pos].getNum();
        if (m_tags[pos].getName() == "LOCATION_Z")
            location.z = m_tags[pos].getNum();
        if (m_tags[pos].getName() == "LOCATION_A")
            location.area = m_tags[pos].getNum();
...
void FileLoader::loadMap(unsigned int& pos) {
    ...
    for (unsigned int zz = 0; zz < m_engine->state()->map()->getMapDepth();
         zz++) {
        for (unsigned int yy = 0; yy < m_engine->state()->map()->getMapHeight();
             yy++) {
            for (unsigned int xx = 0;
                 xx < m_engine->state()->map()->getMapWidth(); xx++) {
                m_engine->state()
                    ->map()
                    ->getTile(Location(xx, yy, zz))
                    .lastVisited = m_tags[pos++].getNum();
            }
        }
    }
}

ComponentBase* FileLoader::loadComponent(unsigned int& pos,
    ...
    if (component == "WEARABLE") {
        ...
        retval->position =
            static_cast<WearablePosition>(m_tags[pos++].getNum());
    ...
    if (component == "WIELDABLE") {
        ...
        retval->position =
            static_cast<WieldablePosition>(m_tags[pos++].getNum());
        return retval;
    }
