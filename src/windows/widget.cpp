...
void Widget::realign(unsigned int xOrigin, unsigned int yOrigin,
    ...
    switch (this->getVerticalAlign()) {
        ...
        case VerticalAlign::Centre: {
            this->m_yPos = yOrigin + this->m_relativeYPos +
                           (parentHeight / 2 - this->getHeight() / 2);
            break;
        }
        case VerticalAlign::Bottom: {
            this->m_yPos = yOrigin + parentHeight - this->getHeight() -
                           this->m_relativeYPos;
            break;
        }
    }
    switch (this->getHorizontalAlign()) {
        ...
        case HorizontalAlign::Centre: {
            this->m_xPos = xOrigin + this->m_relativeXPos +
                           (parentWidth / 2 - this->getWidth() / 2);
            break;
        }
        case HorizontalAlign::Right: {
            this->m_xPos =
                xOrigin + parentWidth - this->getWidth() - this->m_relativeXPos;
            break;
        }
    }
    // std::cout << "width: " << parentWidth << " height: " << parentHeight <<
    // std::endl; std::cout << "offX: " << xOrigin << " offY: " << yOrigin <<
    // std::endl; std::cout << "XPos: " << m_xPos << " YPos: " << m_yPos <<
    // std::endl;
}

Widget* Widget::drawString(unsigned int x, unsigned int y, const char* text,
                           Color fg, Color bg) {
...
Widget* Widget::drawBorder(unsigned int x, unsigned int y, unsigned int width,
                           unsigned int height) {
    m_graphics->drawBorder(m_yPos + y, m_xPos + x, height - 2, width - 2);
    return this;
}

Widget* Widget::clearArea(unsigned int x, unsigned int y, unsigned int width,
                          unsigned int height) {
