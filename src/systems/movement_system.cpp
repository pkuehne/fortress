...
void MovementSystem::handleEvent(const Event* event) {
    switch (event->getType()) {
        case EVENT_MOVE_ENTITY: {
            const MoveEntityEvent* l_event =
                static_cast<const MoveEntityEvent*>(event);
            ...
            // Check if we're running into a collidable or stairs, etc
            {
                ...
                for (EntityId l_target : l_targets) {
                    ...
                    StairComponent* l_stair =
                        m_engine->state()->components()->get<StairComponent>(
                            l_target);
                    if (l_stair && l_stair->target &&
                        l_entity == m_engine->state()->player()) {
                        l_newLocation =
                            m_engine->state()->location(l_stair->target);
                    }
                }
            }

            getEngine()->state()->entityManager()->setLocation(l_event->entity,
                                                               l_newLocation);
